...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetchBackend("/api/v1/salary/benchmark", {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: data.detail || data.error || "Failed to load salary benchmark" }, { status: response.status });
    }
