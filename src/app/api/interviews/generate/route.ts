...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetchBackend("/api/v1/interviews/generate", {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: data.detail || data.error || "Failed to generate interview pack" }, { status: response.status });
    }
